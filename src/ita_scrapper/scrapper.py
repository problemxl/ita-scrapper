...
from .exceptions import ITAScrapperError, NavigationError, ParseError, TimeoutError
from .models import (
    Flight,
    FlightResult,
    FlightSegment,
...
class ITAScrapper:
    ...
    async def search_multi_city(
        ...
        Returns:
            FlightResult containing found flights
        """
        logger.info(f"Searching multi-city flights with {len(search_params.segments)} segments")
    ...
    async def _navigate_to_flights(self):
        ...
        try:
            ...
            response = await self._page.goto(self.base_url, wait_until='domcontentloaded', timeout=30000)
            
            if response and response.status >= 400:
                raise NavigationError(f"HTTP {response.status} error accessing {self.base_url}")
            
            # Wait a bit for JavaScript to load
            await self._page.wait_for_timeout(5000)
            
            # Take a screenshot for debugging
            await self._page.screenshot(path=f"debug_{'matrix' if self.use_matrix else 'google'}.png")
    ...
    async def _fill_matrix_form(self, params: SearchParams):
        ...
        try:
            ...
            for selector in origin_selectors:
                try:
                    origin_input = await self._page.wait_for_selector(selector, timeout=3000)
            ...
            for selector in destination_selectors:
                try:
                    destination_input = await self._page.wait_for_selector(selector, timeout=3000)
    ...
    async def _handle_matrix_dates(self, params: SearchParams):
        ...
        try:
            ...
            await self._set_matrix_date(params.departure_date, 'Start date', is_departure=True)
            
            # Handle return date if needed
            if params.return_date:
                await self._set_matrix_date(params.return_date, 'End date', is_departure=False)
            else:
        ...
        try:
            if is_round_trip:
                ...
                for selector in round_trip_selectors:
                    try:
                        round_trip_tab = await self._page.wait_for_selector(selector, timeout=2000)
                        if round_trip_tab:
                            await round_trip_tab.click()
                            logger.info(f"Selected Round Trip tab with selector: {selector}")
                            await self._page.wait_for_timeout(500)
                            return
                    except:
                        continue
                        
                logger.debug("Could not find Round Trip tab, assuming it's already selected as default")
                ...
                for selector in one_way_selectors:
                    try:
                        one_way_tab = await self._page.wait_for_selector(selector, timeout=2000)
                        if one_way_tab:
                            ...
                            if is_visible and is_enabled:
                                ...
                                logger.info(f"Selected One Way tab with selector: {selector}")
                                await self._page.wait_for_timeout(500)
                                
                                # Verify the click worked
                                aria_selected = await one_way_tab.get_attribute('aria-selected')
                                if aria_selected == 'true':
                                    logger.info("One Way tab successfully selected")
                                    return
                                else:
                                    logger.warning(f"One Way tab click didn't work, aria-selected: {aria_selected}")
                            else:
                                logger.debug(f"One Way tab not clickable: visible={is_visible}, enabled={is_enabled}")
                    except Exception as e:
                        logger.debug(f"One Way selector {selector} failed: {e}")
                        continue
                        
                # If all selectors failed, try JavaScript approach as fallback
                logger.warning("All One Way selectors failed, trying JavaScript fallback")
    ...
    async def _set_matrix_date(self, target_date: date, placeholder: str, is_departure: bool):
        ...
        try:
            ...
            if not date_input:
                ...
                for input_elem in all_inputs:
                    try:
                        ...
                        if any(keyword in (placeholder + class_name).lower() for keyword in ['date', 'depart', 'return']):
                            if 'airport' not in (placeholder + class_name).lower():
            ...
            if not date_input:
                raise Exception(f"Could not find {'departure' if is_departure else 'return'} date input with any selector")
            
            logger.info(f"Found {'departure' if is_departure else 'return'} date input with selector: {successful_selector}")
            ...
            logger.info(f"Typed {'departure' if is_departure else 'return'} date: {formatted_date}")
            ...
            logger.warning(f"Failed to set {'departure' if is_departure else 'return'} date: {e}")
    ...
    async def _submit_matrix_search(self):
        ...
        try:
            ...
            for selector in search_selectors:
                try:
                    search_button = await self._page.wait_for_selector(selector, timeout=2000)
    ...
    async def _fill_google_form(self, params: SearchParams):
        ...
        try:
            ...
            # Fill departure date
            try:
                await self._page.fill('input[placeholder="Departure"]', params.departure_date.strftime('%m/%d/%Y'))
    ...
    async def _submit_google_search(self):
        ...
        try:
            ...
            if self.use_matrix:
                ...
                await self._page.wait_for_selector('[data-testid="flight-card"]', timeout=30000)
    ...
    async def _set_date(self, target_date: date, is_departure: bool):
        ...
        if self.use_matrix:
            ...
            date_input = 'input[placeholder*="Departure"]' if is_departure else 'input[placeholder*="Return"]'
    ...
    async def _parse_flight_results(self, max_results: int) -> List[Flight]:
        ...
        try:
            if self.use_matrix and self._parser:
                ...
                flights = await self._parser.parse_flight_results(self._page, max_results)
                
                if flights:
                    logger.info(f"Enhanced parser found {len(flights)} flights")
                    return flights
                else:
                    logger.warning("Enhanced parser found no flights, falling back to basic parsing")
            ...
            if self.use_matrix:
                ...
                if not flight_cards:
                    ...
                    if 'no flights' in page_text.lower() or 'no results' in page_text.lower():
                    ...
                    for selector in broad_selectors:
                        elements = await self._page.query_selector_all(selector)
                        if len(elements) > 20:  # Arbitrary threshold for "lots of elements"
                            flight_cards = elements[:max_results]
                            logger.info(f"Using broad selector {selector}, found {len(elements)} elements")
                            break
                
            else:
                # Google Flights parsing (unchanged)
                await self._page.wait_for_selector('[data-testid="flight-card"]', timeout=30000)
                flight_cards = await self._page.query_selector_all('[data-testid="flight-card"]')
            ...
            if not flights and self.use_matrix:
                logger.warning("No flights parsed from ITA Matrix, generating demo data")
    ...
    async def _parse_flight_card(self, card) -> Optional[Flight]:
        ...
        try:
            if self.use_matrix:
                ...
                price_clean = price_text.replace('USD', '').replace('$', '').replace(',', '').strip()
                price = Decimal(price_clean)
                
                # Extract airline info
                airline_element = await card.query_selector('.airline')
                airline_name = await airline_element.inner_text() if airline_element else "Unknown"
                
                # Extract duration
                duration_element = await card.query_selector('.duration')
                duration_text = await duration_element.inner_text() if duration_element else "2h 0m"
    ...
    async def _check_site_accessibility(self) -> bool:
        ...
        try:
            ...
            response = await self._page.goto(self.base_url, wait_until='domcontentloaded', timeout=15000)
            ...
            blocking_indicators = [
                "blocked", "captcha", "robot", "automation", "bot",
            ...
            if self.use_matrix and "matrix.itasoftware.com" not in url:
                ...
                return False
            elif not self.use_matrix and "google.com" not in url:
    ...
    async def _get_demo_flight_results(self, params: "SearchParams", max_results: int = 3) -> List[Flight]:
        ...
        for i in range(min(max_results, 3)):
            ...
            departure_time = datetime.combine(params.departure_date, datetime.min.time().replace(hour=8 + i*6, minute=30))
            ...
            flight = Flight(
                ...
                stops=stops,
                is_refundable=params.cabin_class in [CabinClass.BUSINESS, CabinClass.FIRST],
    ...
    async def _get_demo_price_calendar(self, origin: str, destination: str, departure_month: date, cabin_class: CabinClass) -> "PriceCalendar":
        ...
        for day in range(1, 32):
            try:
                ...
                final_price = Decimal(str(int(base_price * weekend_multiplier * price_variation)))
