...
from .models import CabinClass, TripType
...
@main.command()
@click.option('--origin', '-o', required=True, help='Origin airport code (e.g., JFK, NYC)')
@click.option('--destination', '-d', required=True, help='Destination airport code (e.g., LAX, SFO)')
@click.option('--departure-date', '-dep', required=True, help='Departure date (YYYY-MM-DD)')
...
@click.option('--cabin-class', type=click.Choice(['ECONOMY', 'PREMIUM_ECONOMY', 'BUSINESS', 'FIRST']), 
...
def search(origin: str, destination: str, departure_date: str, return_date: Optional[str], 
           adults: int, children: int, infants: int, cabin_class: str, headless: bool, 
           format: str, limit: int):
                ...
                if format == 'json':
                    ...
                    output = {
                        'search_params': {
                            ...
                            'return_date': result.search_params.return_date.isoformat() if result.search_params.return_date else None,
                    ...
                    click.echo(f"Passengers: {adults} adult(s), {children} child(ren), {infants} infant(s)")
                    ...
                    for i, flight in enumerate(result.flights[:limit], 1):
                        ...
                        if flight.duration:
                            duration_str = f"{flight.duration} min" if isinstance(flight.duration, int) else flight.duration
                            click.echo(f"⏱️  Duration: {duration_str}")
                        
                        if flight.stops is not None:
                            stops_str = "Direct" if flight.stops == 0 else f"{flight.stops} stop(s)"
...
@main.command()
@click.argument('text')
@click.option('--type', 'data_type', type=click.Choice(['price', 'duration', 'time', 'airport']),
...
def parse(text: str, data_type: str, reference_date: Optional[str]):
    ...
    from .utils import parse_price, parse_duration, parse_time, validate_airport_code
    
    try:
        if data_type == 'price':
            result = parse_price(text)
            click.echo(f"Parsed price: ${result}" if result else "Could not parse price")
