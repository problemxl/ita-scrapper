...
async def matrix_basic_search():
    ...
    try:
        ...
        async with ITAScrapper(headless=True, use_matrix=True, demo_mode=True) as scrapper:
            ...
            print(f"Search: {result.search_params.origin} → {result.search_params.destination}")
...
async def compare_matrix_vs_google():
    ...
    try:
        ...
        async with ITAScrapper(headless=True, use_matrix=True, demo_mode=True) as matrix_scrapper:
        ...
        async with ITAScrapper(headless=True, use_matrix=False, demo_mode=True) as google_scrapper:
...
async def matrix_advanced_search():
    ...
    try:
        async with ITAScrapper(headless=True, use_matrix=True, demo_mode=True) as scrapper:
            ...
            print(f"Advanced search: {result.search_params.origin} → {result.search_params.destination}")
        ...
        async with ITAScrapper(headless=True, use_matrix=True, demo_mode=True) as scrapper:
            ...
            for i, entry in enumerate(cheapest_dates, 1):
                if entry.price:
                    ...
                    print(f"  {i}. {entry.date.strftime('%Y-%m-%d')} ({day_name}): ${entry.price}")
