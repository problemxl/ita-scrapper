...
async def basic_flight_search():
    ...
    try:
        async with ITAScrapper(headless=True) as scrapper:
            ...
            print(f"Search from {result.search_params.origin} to {result.search_params.destination}")
            ...
            if result.flights:
                ...
                if fastest and fastest != cheapest:
                    ...
                    print(f"   Duration: {fastest.total_duration_minutes // 60}h {fastest.total_duration_minutes % 60}m")
                    print()
                ...
                for i, flight in enumerate(result.flights[:5], 1):
                    ...
                    print(f"  {i}. ${flight.price} - {duration_h}h {duration_m}m - {', '.join(flight.airlines)}")
...
async def one_way_search():
    ...
    try:
        async with ITAScrapper(headless=True) as scrapper:
            ...
            print(f"One-way search: {result.search_params.origin} → {result.search_params.destination}")
            print(f"Passengers: {result.search_params.adults} adults, {result.search_params.children} children")
...
async def business_class_search():
    ...
    try:
        async with ITAScrapper(headless=True) as scrapper:
            ...
            print(f"Business class search: {result.search_params.origin} ↔ {result.search_params.destination}")
            ...
            if result.flights:
                ...
                for i, flight in enumerate(result.flights[:3], 1):
                    ...
                    print(f"  {i}. ${flight.price:,} - {duration_h}h {duration_m}m - {stops_text}")
...
async def main():
    ...
    print("Note: These examples use mock data. Real implementation would scrape Google Flights.")
    print()
