...
async def basic_flight_search_demo():
    ...
    async with ITAScrapper(demo_mode=True) as scrapper:
        ...
        for i, flight in enumerate(result.flights, 1):
            ...
            print(f"   {flight.departure_time.strftime('%H:%M')} → {flight.arrival_time.strftime('%H:%M')}")
            print(f"   {first_segment.departure_airport.code} → {last_segment.arrival_airport.code}")
            print(f"   Duration: {flight.total_duration_minutes // 60}h {flight.total_duration_minutes % 60}m")
            print(f"   Stops: {flight.stops}")
            print(f"   Price: ${flight.price}")
            if len(flight.segments) > 1:
                print(f"   Segments:")
                for j, segment in enumerate(flight.segments, 1):
                    print(f"     {j}. {segment.departure_airport.code} → {segment.arrival_airport.code}")
...
async def one_way_flight_demo():
    ...
    async with ITAScrapper(demo_mode=True) as scrapper:
        ...
        print(f"Cheapest: {cheapest.segments[0].airline.name} {cheapest.segments[0].flight_number} - ${cheapest.price}")
        
        fastest = min(result.flights, key=lambda f: f.total_duration_minutes)
        print(f"Fastest: {fastest.segments[0].airline.name} {fastest.segments[0].flight_number} - {fastest.total_duration_minutes // 60}h {fastest.total_duration_minutes % 60}m")
...
async def business_class_demo():
    ...
    async with ITAScrapper(demo_mode=True) as scrapper:
        ...
        for flight in result.flights[:2]:  # Show first 2
            print(f"• {flight.segments[0].airline.name} {flight.segments[0].flight_number}")
