...
async def enhanced_flight_search_demo():
    ...
    try:
        ...
        async with ITAScrapper(
            ...
            print(f"🛫 Route: {result.search_params.origin} → {result.search_params.destination}")
            ...
            if result.flights:
                ...
                for i, flight in enumerate(result.flights, 1):
                    ...
                    print(f"⏱️  Total Duration: {flight.total_duration_minutes // 60}h {flight.total_duration_minutes % 60}m")
                    ...
                    for j, segment in enumerate(flight.segments, 1):
                        print(f"  Segment {j}: {segment.departure_airport.code} → {segment.arrival_airport.code}")
                        print(f"    🏢 Airline: {segment.airline.name} ({segment.airline.code})")
                        print(f"    🎫 Flight: {segment.flight_number}")
                        print(f"    🛫 Departure: {segment.departure_time.strftime('%I:%M %p on %b %d')}")
                        print(f"    🛬 Arrival: {segment.arrival_time.strftime('%I:%M %p on %b %d')}")
                        print(f"    ⏱️  Duration: {segment.duration_minutes // 60}h {segment.duration_minutes % 60}m")
                        if segment.stops > 0:
                ...
                if result.cheapest_flight:
                    ...
                    print(f"   Duration: {cheapest.total_duration_minutes // 60}h {cheapest.total_duration_minutes % 60}m")
                    print(f"   Airlines: {', '.join(cheapest.airlines)}")
                    print(f"   Stops: {cheapest.stops}")
                
                if result.fastest_flight:
                    fastest = result.fastest_flight
                    print(f"⚡ FASTEST: {fastest.total_duration_minutes // 60}h {fastest.total_duration_minutes % 60}m")
                ...
                print(f"   Range: {min_duration // 60}h {min_duration % 60}m - {max_duration // 60}h {max_duration % 60}m")
                print(f"   Time saved: {(max_duration - min_duration) // 60}h {(max_duration - min_duration) % 60}m (choosing fastest)")
        ...
        for origin, dest, description in routes:
            ...
            try:
                async with ITAScrapper(
                    ...
                    if result_ita.flights:
                        ...
                        print(f"   🏢 Airlines: {', '.join(result_ita.flights[0].airlines)}")
            ...
            try:
                async with ITAScrapper(
                    ...
                    print(f"✅ Google Flights: {len(result_google.flights)} flights found")
                    if result_google.flights:
                        google_cheapest = result_google.cheapest_flight.price
                        print(f"   💰 Cheapest: ${google_cheapest}")
                        print(f"   🏢 Airlines: {', '.join(result_google.flights[0].airlines)}")
...
async def main():
    ...
    print(f"\n{'='*60}")
    user_input = input("🔄 Do you want to compare ITA Matrix vs Google Flights parsing? (y/N): ")
