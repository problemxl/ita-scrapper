            ...
            if result.flights:
                ...
                for i, flight in enumerate(result.flights, 1):
                    ...
                    for j, segment in enumerate(flight.segments):
                        print(f"   Segment {j+1}: {segment.departure_airport.code} → {segment.arrival_airport.code}")
                        print(f"     {segment.airline.name} {segment.flight_number}")
                        print(f"     Dep: {segment.departure_time.strftime('%I:%M %p %b %d')}")
                        print(f"     Arr: {segment.arrival_time.strftime('%I:%M %p %b %d')}")
                
                # Test flight filtering and sorting
                print(f"\n💰 Cheapest flight: ${result.cheapest_flight.price}")
                print(f"⚡ Fastest flight: {result.fastest_flight.total_duration_minutes // 60}h {result.fastest_flight.total_duration_minutes % 60}m")
...
async def test_html_parsing():
    ...
    example_html_path = Path(__file__).parent.parent / "src" / "ita_scrapper" / "example.html"
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            for i, (tooltip_id, tooltip_text) in enumerate(list(tooltip_data.items())[:5]):
            ...
            if times_found:
                ...
                for time_info in times_found[:3]:  # Show first 3
                    print(f"   {time_info['airport']}: {time_info['time']} {time_info['date']}")
